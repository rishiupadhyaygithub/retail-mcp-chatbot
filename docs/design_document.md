...
- **The handshake.** Client and server exchange `initialize` / `initialized`. Each sends its `protocolVersion`, its `capabilities`, and some info about itself. If the versions or capabilities don't match, the session fails right here — which is exactly the "initialization failure" case I have to handle without crashing. **What my server actually reports on the wire is `2025-11-25`, not `2026-07-28`, and that is worth being exact about.** In the pinned SDK (`mcp==2.0.0`) `LATEST_PROTOCOL_VERSION` is `2026-07-28` — that is the revision its type model implements — but the negotiable set is `HANDSHAKE_PROTOCOL_VERSIONS = ('2024-11-05', '2025-03-26', '2025-06-18', '2025-11-25')`, so `2025-11-25` is the newest version any handshake can settle on. I checked the older pin too: `mcp==1.27.2` negotiates the same `2025-11-25`. So "we target 2026-07-28" is a statement about which spec revision the implementation conforms to, not about the number in the `initialize` result, and any conformance report that quotes a handshake version should quote `2025-11-25`.
...
flowchart TD
  subgraph build["Built once"]
    d["22 documents"] --> ch["Split on ## headings"] --> pk["Pack adjacent sections<br/>of the same document<br/>to a ~100-word target"] --> em1["Embed each chunk<br/>(local bge-small)"] --> db[("ChromaDB<br/>retail_docs_heading · 97<br/>retail_docs_packed · 33")]
  end
...
**Vector store.** ChromaDB, local and on disk. Ingestion writes two collections for the baseline A/B — `retail_docs_heading` and `retail_docs_packed` — and whichever wins on measured Recall@5 is the one collection the server ships against. **Measured outcome: both tied at Recall@5 = 100% (n=11) and both cleared the ≤300 ms latency gate, so the Recall@1 tiebreaker decided it — `heading` 90.9% over `packed` 81.8%. `server/retrieval.py` queries `retail_docs_heading`.** Both collections stay on disk so the A/B is re-runnable as the corpus grows; there is deliberately no third collection copied to a generic name like `retail_docs`, because that would be a second source of truth free to drift from the measured winner. Each record is `{content, embedding, source, section, chunk_id, document_type}`. The `chunk_id` looks like `retail-doc-3:chunk-12` — the exact format from contract v1, so citations line up across all four servers. The `<n>` is assigned once from a sorted list of the 22 documents and reused by both strategies, so the two scorecards stay comparable and the same passage cites the same way whichever collection answered. Similarity is **cosine**, and I normalize scores to 0–1 before returning them (also contract v1, so they're comparable across servers whatever store each intern uses).
