...
Generated 2026-08-18 13:02:05 +04 by `eval/harness.py`. Phase 1 (documents) only, retrieval only.
...
| Latency: query embedding, WARM p50 | timed as its own stage, nearest-rank, n=10 | part of the <= 300 ms retrieval budget | 68.8 ms | 34.8 ms | - |
| Latency: query embedding, WARM p95 | timed as its own stage, nearest-rank, n=10 | part of the <= 300 ms retrieval budget | 313.4 ms | 259.9 ms | - |
| Latency: vector search, WARM p50 | timed as its own stage, nearest-rank, n=10 | part of the <= 300 ms retrieval budget | 2.1 ms | 2.0 ms | - |
| Latency: vector search, WARM p95 | timed as its own stage, nearest-rank, n=10 | part of the <= 300 ms retrieval budget | 5.2 ms | 23.5 ms | - |
| Latency: retrieval total, WARM p50 | embed + search, cold query discarded, n=10 | <= 300 ms | 70.6 ms | 37.4 ms | **PASS** |
| Latency: retrieval total, WARM p95 | embed + search, cold query discarded, n=10 | <= 300 ms | 317.7 ms | 263.1 ms | heading: **FAIL** / packed: **PASS** |
| Latency: retrieval total, COLD | first query after model load (Q1), 1 sample | reported separately, no gate | 194.3 ms (embed 189.9 + search 4.4) | 32.1 ms (embed 30.2 + search 1.9) | - |
...
| Tokens: NAIVE BASELINE (compact) | verbatim top-5 tool result, compact JSON, mean n=11 | BASELINE - the number to beat | 586 tokens/query | 1137 tokens/query | BASELINE |
| Tokens: NAIVE BASELINE (verbose) | verbatim top-5 tool result, indented JSON, mean n=11 | BASELINE - worst case | 732 tokens/query | 1282 tokens/query | BASELINE |
...
**Measured winner: `packed`** — `packed` tied on Recall@5 with `heading`, so the tie was broken on Recall@1 (90.9% vs 100.0%); heading excluded: p95 latency > 300 ms. Winner decided by Recall@5, latency gate (p95 ≤ 300 ms), then Recall@1 — not by preference.

### Per-question detail - `heading` (n=11)

| # | Question | Top-1 retrieved | @1 | @5 | Retrieval ms | Tokens (compact) | Tokens (verbose) |
|---|---|---|---|---|---|---|---|
| 1 | how long till they get their money back on a return? | `amazon/refund_timelines.md` | Y | Y | 194.3 *(cold)* | 591 | 736 |
| 2 | customer's parcel never showed up — what's our process? | `amazon/order_tracking.md` | Y | Y | 167.8 | 592 | 737 |
| 3 | can they send back opened electronics? | `target/returns.md` | Y | Y | 134.7 | 513 | 658 |
| 4 | what's covered under warranty and for how long? | `ikea/warranty_terms.md` | Y | Y | 46.7 | 770 | 915 |
| 5 | they want to return after 40 days, are we allowed? | `amazon/returns.md` | Y | Y | 42.9 | 528 | 673 |
| 6 | returns window AND who pays return shipping? | `amazon/returns.md` | Y | Y | 317.7 | 536 | 681 |
| 7 | does every store charge a 'restocking fee' on returns? | `bestbuy/returns.md` | Y | Y | 148.8 | 577 | 722 |
| 14 | I was charged twice — is that allowed and did it actually happen? (document half only) | `amazon/charged_twice.md` | Y | Y | 151.1 | 626 | 772 |
| 15 | can they return order [REF] — what's the window and is it eligible? (document half only) | `amazon/returns.md` | Y | Y | 70.6 | 512 | 658 |
| 16 | parcel split into two — is partial delivery covered, and what shipped? (document half only) | `amazon/charged_twice.md` | Y | Y | 64.3 | 605 | 751 |
| 17 | refund on [REF] — how long should it take and did it go through? (document half only) | `amazon/refund_timelines.md` | Y | Y | 59.9 | 600 | 746 |

### Per-question detail - `packed` (n=11)

| # | Question | Top-1 retrieved | @1 | @5 | Retrieval ms | Tokens (compact) | Tokens (verbose) |
|---|---|---|---|---|---|---|---|
| 1 | how long till they get their money back on a return? | `amazon/refund_timelines.md` | Y | Y | 32.1 *(cold)* | 1089 | 1234 |
| 2 | customer's parcel never showed up — what's our process? | `amazon/order_tracking.md` | Y | Y | 34.7 | 1058 | 1203 |
| 3 | can they send back opened electronics? | `target/returns.md` | Y | Y | 31.4 | 1191 | 1336 |
| 4 | what's covered under warranty and for how long? | `ikea/warranty_terms.md` | Y | Y | 32.0 | 1041 | 1186 |
| 5 | they want to return after 40 days, are we allowed? | `bestbuy/returns.md` | Y | Y | 118.9 | 1189 | 1334 |
| 6 | returns window AND who pays return shipping? | `amazon/returns.md` | Y | Y | 263.1 | 1187 | 1332 |
| 7 | does every store charge a 'restocking fee' on returns? | `bestbuy/returns.md` | Y | Y | 39.0 | 1131 | 1276 |
| 14 | I was charged twice — is that allowed and did it actually happen? (document half only) | `amazon/charged_twice.md` | Y | Y | 38.4 | 1118 | 1264 |
| 15 | can they return order [REF] — what's the window and is it eligible? (document half only) | `amazon/returns.md` | Y | Y | 38.1 | 1187 | 1333 |
| 16 | parcel split into two — is partial delivery covered, and what shipped? (document half only) | `target/delivery.md` | n | Y | 37.4 | 1097 | 1243 |
| 17 | refund on [REF] — how long should it take and did it go through? (document half only) | `amazon/refund_timelines.md` | Y | Y | 35.8 | 1214 | 1360 |
