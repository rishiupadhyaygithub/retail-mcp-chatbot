...
Generated 2026-08-18 15:28:37 +04 by `eval/harness.py`. Phase 1 (Documents) + Phase 2 (Records).
...
| Latency: query embedding, WARM p50 | timed as its own stage, nearest-rank, n=10 | part of the <= 300 ms retrieval budget | 82.6 ms | 33.0 ms | - |
| Latency: query embedding, WARM p95 | timed as its own stage, nearest-rank, n=10 | part of the <= 300 ms retrieval budget | 218.4 ms | 35.5 ms | - |
| Latency: vector search, WARM p50 | timed as its own stage, nearest-rank, n=10 | part of the <= 300 ms retrieval budget | 2.8 ms | 2.0 ms | - |
| Latency: vector search, WARM p95 | timed as its own stage, nearest-rank, n=10 | part of the <= 300 ms retrieval budget | 24.2 ms | 3.7 ms | - |
| Latency: retrieval total, WARM p50 | embed + search, cold query discarded, n=10 | <= 300 ms | 106.8 ms | 35.4 ms | **PASS** |
| Latency: retrieval total, WARM p95 | embed + search, cold query discarded, n=10 | <= 300 ms | 223.9 ms | 37.5 ms | **PASS** |
| Latency: retrieval total, COLD | first query after model load (Q1), 1 sample | reported separately, no gate | 272.0 ms (embed 269.0 + search 3.0) | 38.6 ms (embed 35.3 + search 3.3) | - |
...
| Latency: structured query, WARM p50 | direct SQLite query execution time, n=11 | <= 100 ms | 0.33 ms | 0.33 ms | **PASS** |
| Latency: structured query, WARM p95 | direct SQLite query execution time, n=11 | <= 100 ms | 5.44 ms | 5.44 ms | **PASS** |
...
|---|---|---|---|---|---|
| 8 | status of order ORD-9021? | `kb_retail_query_orders` | `order_id='ORD-9021'` | 5.44 ms | **PASS** |
| 9 | did order ORD-9021 actually ship? | `kb_retail_query_shipments` | `order_id='ORD-9021'` | 0.33 ms | **PASS** |
| 10 | list open returns for customer CUST-103 | `kb_retail_query_returns` | `customer_id='CUST-103', status='refund_processing'` | 0.21 ms | **PASS** |
| 11 | how many orders has Alex Rivera (CUST-101) placed this year? | `kb_retail_query_customer` | `customer_id='CUST-101'` | 0.55 ms | **PASS** |
| 12 | how much has Marcus Vance (CUST-103) been refunded this year? | `kb_retail_query_customer` | `customer_id='CUST-103'` | 0.55 ms | **PASS** |
| 13 | which line items in order ORD-9021 were delivered? | `kb_retail_query_shipments` | `order_id='ORD-9021'` | 0.33 ms | **PASS** |
| 14 | I was charged twice — is that allowed and did it actually happen? (record half) | `kb_retail_query_orders` | `customer_id='CUST-101'` | 1.15 ms | **PASS** |
| 15 | can they return order ORD-9031 — what's the window and is it eligible? (record half) | `kb_retail_query_orders` | `order_id='ORD-9031'` | 0.40 ms | **PASS** |
| 16 | parcel split into two — is partial delivery covered, and what shipped? (record half) | `kb_retail_query_shipments` | `order_id='ORD-9021'` | 0.18 ms | **PASS** |
| 17 | refund on return RET-701 — how long should it take and did it go through? (record half) | `kb_retail_query_returns` | `return_id='RET-701'` | 0.15 ms | **PASS** |
| 28 | status of order ORD-99999999? | `kb_retail_query_orders` | `order_id='ORD-99999999'` | 0.13 ms | **PASS** |
...
|---|---|---|---|---|---|---|---|
| 1 | how long till they get their money back on a return? | `amazon/refund_timelines.md` | Y | Y | 272.0 *(cold)* | 684 | 859 |
| 2 | customer's parcel never showed up — what's our process? | `amazon/order_tracking.md` | Y | Y | 121.5 | 697 | 872 |
| 3 | can they send back opened electronics? | `target/returns.md` | Y | Y | 109.5 | 596 | 771 |
| 4 | what's covered under warranty and for how long? | `ikea/warranty_terms.md` | Y | Y | 49.4 | 915 | 1090 |
| 5 | they want to return after 40 days, are we allowed? | `amazon/returns.md` | Y | Y | 48.8 | 619 | 794 |
| 6 | returns window AND who pays return shipping? | `amazon/returns.md` | Y | Y | 72.7 | 613 | 788 |
| 7 | does every store charge a 'restocking fee' on returns? | `bestbuy/returns.md` | Y | Y | 77.8 | 686 | 861 |
| 14 | I was charged twice — is that allowed and did it actually happen? (document half only) | `amazon/charged_twice.md` | Y | Y | 106.8 | 739 | 915 |
| 15 | can they return order [REF] — what's the window and is it eligible? (document half only) | `amazon/returns.md` | Y | Y | 170.6 | 591 | 767 |
| 16 | parcel split into two — is partial delivery covered, and what shipped? (document half only) | `amazon/charged_twice.md` | Y | Y | 221.2 | 698 | 874 |
| 17 | refund on [REF] — how long should it take and did it go through? (document half only) | `amazon/refund_timelines.md` | Y | Y | 223.9 | 693 | 869 |
### Per-question detail - Document Retrieval `packed` (n=11)

| # | Question | Top-1 retrieved | @1 | @5 | Retrieval ms | Tokens (compact) | Tokens (verbose) |
|---|---|---|---|---|---|---|---|
| 1 | how long till they get their money back on a return? | `amazon/refund_timelines.md` | Y | Y | 38.6 *(cold)* | 1176 | 1351 |
| 2 | customer's parcel never showed up — what's our process? | `amazon/order_tracking.md` | Y | Y | 36.2 | 1153 | 1328 |
| 3 | can they send back opened electronics? | `target/returns.md` | Y | Y | 35.6 | 1282 | 1457 |
| 4 | what's covered under warranty and for how long? | `ikea/warranty_terms.md` | Y | Y | 32.8 | 1184 | 1359 |
| 5 | they want to return after 40 days, are we allowed? | `bestbuy/returns.md` | Y | Y | 33.8 | 1278 | 1453 |
| 6 | returns window AND who pays return shipping? | `amazon/returns.md` | Y | Y | 35.8 | 1278 | 1453 |
| 7 | does every store charge a 'restocking fee' on returns? | `bestbuy/returns.md` | Y | Y | 37.5 | 1232 | 1407 |
| 14 | I was charged twice — is that allowed and did it actually happen? (document half only) | `amazon/charged_twice.md` | Y | Y | 37.2 | 1227 | 1403 |
| 15 | can they return order [REF] — what's the window and is it eligible? (document half only) | `amazon/returns.md` | Y | Y | 34.9 | 1270 | 1446 |
| 16 | parcel split into two — is partial delivery covered, and what shipped? (document half only) | `target/delivery.md` | n | Y | 35.4 | 1196 | 1372 |
| 17 | refund on [REF] — how long should it take and did it go through? (document half only) | `amazon/refund_timelines.md` | Y | Y | 35.0 | 1303 | 1479 |
