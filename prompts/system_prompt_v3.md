...
**Version:** 3
**Status:** active
**First used:** 2026-08-27
**Loaded by:** `client/loop.py` (`load_system_prompt`)

Same as v2 except the citation rule. v2 illustrated the citation format with
`[retail: Returns and Refunds Policy]` — a document that does not exist in the
corpus. Q17 was scored as the model fabricating a citation; it was reproducing
the example it had been given. A worked example of a citation is therefore the
one thing this section must not contain, so v3 gives the shape and forbids any
title that did not come back in a tool result.

Retains the rules for Phase 2 (structured operational records query tools) and
Phase 3 (state-changing actions).
...
CITATIONS
- Every claim names its source in square brackets, in this shape:
  [SERVER: the tool result's own `source` value]
- Never write a title that did not come back in a tool result this turn. If no
  result carries a `source`, cite nothing and say you don't know.
